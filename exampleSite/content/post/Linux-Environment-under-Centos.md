...
description: CentOS 开发环境的软件安装合集
...
- Linux
- Example
title: 在CentOS 7 上打造Linux开发环境
comment : true
---

>前段时间,组里的一台GPU服务器宕机了,在我的强烈要求下,管理老师让机器售后服务方安装了CentOS 7系统;然后就直接甩锅给我,让我自己去安装
相应的软件...
<!--more-->
>看到一篇比较好的博客,转载并更新了一些东西,形成了这篇博客,以期记录下相关过程以及一些坑。。。

>本文大部分内容来自于[SeisMan](http://seisman.info/linux-environment-for-seismology-research.html),仅做了一点点修改。

阅读须知：

1.  **不推荐刚接触 Linux 的人使用 CentOS 发行版**；
2.  本文只为记录，但要求读者掌握 Linux 基础知识；
3.  本文所有操作均在 CentOS 7 下完成，其他发行版或多或少与 CentOS 7 不同，因而仅供参考；

## 安装 CentOS
> 由于已经由售后安装好了系统,系统安装部分未实验。
...
具体参见《[CentOS7 下的软件安装方法与策略](http://seisman.info/how-to-install-softwares-under-centos-7.html)》。
...
~~提供的一个 yum 插件。使用该插件后用 yum 安装软件时可以并行下载，大大提高了软件的下载速度，减少了下载的等待时间:~~
    ...
    sudo yum install yum-axelget        #安装
    sudo yum uninstall yum-axelget      #卸载

安装该插件的同时会安装另一个软件 axel。axel 是一个并行下载工具，在下载 http、ftp 等简单协议的文件时非常好用。
### 安装 yum-fastestmirror
`fastestmirror`会自动选择速度比较快的站点,亲测效果很好。

    sudo yum install yum-fastestmirror        #安装
    sudo yum uninstall yum-fastestmirror      #卸载
...
```
> 由于服务器上已经mount了intel编译器,不需要我去安装了;如果需要,可以去intel官网上申请并下载安装,然后设置compiler、mkl的环境即可。    
...
### 显卡驱动
由于显卡驱动依赖内核版本,应该在内核升级之后再安装,否则内核升级之后需要重新安装。
在Nvidia官网上下载相应的型号以及对应系统版本的安装包,一键安装即可。可通过运行`nvidia-smi`查看是否安装成功。
...
### GPU 开发环境
在安装了GPU驱动之后,从`Nvidia`网站上下载`cuda`安装包,同样要与GPU型号和系统匹配,一键安装即可,或许会有
提示是否安装驱动,如果前面没有安装,那么这里就可以选择安装了;否则就没有必要安装了,因为这里的版本略老。
安装成功后设置一下`PATH`和`LD_LIBRARY_PATH`:(写进`.bashrc`或者`/etc/profile.d/cuda.sh`(建议)均可)
```bash
...
> yum安装的python版本较老,建议通过源码编译安装

```bash
    tar -xzvf Python-3.6.3.tgz
    cd Python-3.6.3
    ./configure --prefix=/usr/local/Python3 --enable-optimizations --enable-profiling --with-lto --enable-loadable-sqlite-extensions --enable-big-dig
    make
    sudo make install 
```
> 建议: 在configure之前,修改`Module/Setup.dist`:开启 `readline`,开启 `ssl`,并复制为 `Setup`
...
## 日常软件
> 以下部分对于服务器而言基本没什么用,但由于原博文是针对桌面环境,仍然保留,万一以后用的上呢
...
2. 转载
-   2017-12-08: 转载并更新部分内容
-   2017-12-08: 删除elrepo,建议驱动从官网安装;
                删除yum-axelget;
                增加部分软件开发辅助工具和其他工具;
